/**
 * Check the published JSON dataset; invoked by scripts/validate.ts.
 *
 * FAIL: schema errors, forbidden ticker suffixes, or a golden reference
 * below the existing 95% field-match threshold.
 * REVIEW: matching transactions appear in different filings.
 * PASS: neither condition applies. Volume and same-day clusters are notes.
 *
 * These checks do not establish PDF accuracy. Current limitations include
 * permissive date parsing and non-fatal missing goldens/count differences;
 * changing those policies is separate from organizing this module.
 */
import { readdir, readFile } from "fs/promises";
import { join } from "path";
import { VALID_TRANSACTION_TYPES } from "./parsed-rows";
import { isAmountRange } from "../amounts";
import { NEVER_A_SYMBOL, SYMBOL_SHAPE } from "../assets";
...
function validateSchema(tx: Transaction, official: string, index: number): string[] {
  ...
  if (!VALID_TRANSACTION_TYPES.some((type) => type === tx.type)) {
...
export async function validatePublishedData(): Promise<ValidationReport> {
